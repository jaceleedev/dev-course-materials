...
2.  **Ruff**: 파이썬 코드를 검사하고 자동으로 스타일을 교정해주는, 현재 가장 빠르고 강력한 필수 도구입니다. (자세한 설명은 아래에 있습니다.)
3.  **(선택) Material Icon Theme**: 파일 종류에 맞는 예쁜 아이콘을 표시해 줍니다.
...
1.  `Ctrl + N`을 눌러 새 파일을 만들고, 아래 코드를 일부러 지저분하게 붙여넣은 후 `hello.py`로 저장하세요.
2.  `Ctrl + S`로 저장하는 순간, `Ruff`가 작동하여 코드가 자동으로 깔끔하게 정리되는 마법을 경험해 보세요\!
3.  **실행 방법**:
...
![PyCharm New Project](https://www.jetbrains.com/help/pycharm/absolute_path/015091a18ba7277259f9a46328816f1c.png)
...
훌륭합니다\! 이제 여러분은 파이썬 코드를 작성하고 실행할 수 있는 강력한 무기를 갖추었습니다. 다음 토픽부터는 본격적으로 파이썬의 기초 문법을 탐험하며 이 도구들을 능숙하게 사용하는 방법을 익히게 될 것입니다.
